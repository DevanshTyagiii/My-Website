...
const ContactForm = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.ok) {
                // FormSubmit response might be empty or valid JSON, but 200 OK means success
                setStatus("success");
                setFormState({ name: "", email: "", description: "" });
            } else {
                // Try to parse error, but fallback to status text
                const data = await response.json().catch(() => ({}));
                console.error("Submission failed:", data.message || response.statusText);
                setStatus("idle");
